...
function Datepicker() {
	...
	this.regional[''] = { // Default regional settings
		...
		dateFormat: 'mm/dd/yy', // See format options on parseDateTime
		timeFormat: 'hh:ii:ss', // See format options on parseDateTime
...
$.extend(Datepicker.prototype, {
	...
	_newInst: function(target, inline) {
		...
		return {id: id, input: target, // associated target
			...
			selectedHour: 0, selectedMinute: 0, selectedSecond: 0, // current selection
			drawMonth: 0, drawYear: 0, // month being drawn
			drawHour: 0, drawMinute: 0, drawSecond: 0, // current selection
	...
	_doKeyUp: function(event) {
		...
		if (inst.input.val() != inst.lastVal) {
			try {
				var date = $.datepicker.parseDateTime($.datepicker._get(inst, 'dateFormat')+' '+$.datepicker._get(inst, 'timeFormat'),
	...
	/* Action for selecting a new Hour/Minute/Second. */
	_selectTime: function(id, select, period) {
		var target = $(id);
		var inst = this._getInst(target[0]);
		
		inst._selectingMonthYear = false;
		inst['selected' + (period == 'S' ? 'Second' :  (period == 'I' ? 'Minute' : 'Hour'))] =
		inst['draw' + (period == 'S' ? 'Second' :  (period == 'I' ? 'Minute' : 'Hour'))] =
			parseInt(select.options[select.selectedIndex].value,10);
		this._notifyChange(inst);
		this._adjustDate(target);
	},
	...
	_selectDay: function(id, month, year, td) {
		...
		inst.selectedSecond = inst.currentSecond = inst.drawSecond;
	...
	_updateAlternate: function(inst) {
		...
		if (altField) { // update alternate field too
			var altFormat = this._get(inst, 'altFormat') || this._get(inst, 'dateFormat')+' '+this._get(inst, 'timeFormat');
	...
	parseDateTime: function (format, value, settings) {
		...
		var date = this._daylightSavingAdjust(new Date(year, month - 1, day, hour, minute, second));
	...
	_setDateFromField: function(inst, noDefault) {
		...
		try {
			date = this.parseDateTime(dateFormat, dates, settings) || defaultDate;
		} catch (event) {
			this.log(event);
	...
	_determineDate: function(inst, date, defaultDate) {
		...
		var offsetString = function(offset) {
			try {
				return $.datepicker.parseDateTime($.datepicker._get(inst, 'dateFormat')+' '+$.datepicker._get(inst, 'timeFormat'),
			...
			var pattern = /([+-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y|H|i|s)?/g;
	...
	_generateHTML: function(inst) {
		...
		//nbaiwan
		var timePanel = (showTimePanel) ? '<div class="ui-datepicker-timepane ui-widget-content"></div>' : '';
	...
	_generateMonthYearHeader: function(inst, drawSecond, drawMinute, drawHour, drawMonth, drawYear, minDate, maxDate,
		...
		html += '<div class="ui-datepicker-year">';
		html += '<select class="ui-datepicker-hour" ' +
			'onchange="DP_jQuery_' + dpuuid + '.datepicker._selectTime(\'#' + inst.id + '\', this, \'H\');" ' +
			'onclick="DP_jQuery_' + dpuuid + '.datepicker._clickMonthYear(\'#' + inst.id + '\');">';
		...
		html += '</select>';
		html += '&nbsp;:&nbsp;';
		html += '<select class="ui-datepicker-minute" ' +
			'onchange="DP_jQuery_' + dpuuid + '.datepicker._selectTime(\'#' + inst.id + '\', this, \'I\');" ' +
			'onclick="DP_jQuery_' + dpuuid + '.datepicker._clickMonthYear(\'#' + inst.id + '\');">';
		...
		html += '</select>';
		html += '&nbsp;:&nbsp;';
		html += '<select class="ui-datepicker-second" ' +
			'onchange="DP_jQuery_' + dpuuid + '.datepicker._selectTime(\'#' + inst.id + '\', this, \'S\');" ' +
			'onclick="DP_jQuery_' + dpuuid + '.datepicker._clickMonthYear(\'#' + inst.id + '\');">';
	...
	//nbaiwan
	/* Generate the Hour and minute and second. */
	_generateHourMinuteSecond: function(inst, drawHour, drawMinute, minSecond, maxDate,
			secondary, monthNames, monthNamesShort) {
		var html = '<div class="ui-datepicker-timepane">';
		var monthHtml = '';
		// month selection
		if (secondary || !changeMonth)
			monthHtml += '<span class="ui-datepicker-hour">' + monthNames[drawMonth] + '</span>';
		else {
			var inMinYear = (minDate && minDate.getFullYear() == drawYear);
			var inMaxYear = (maxDate && maxDate.getFullYear() == drawYear);
			monthHtml += '<select class="ui-datepicker-month" ' +
