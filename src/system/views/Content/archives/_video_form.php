	...
	?>
	<input type="hidden" name="Archive[class_id][]" value="13" />
...
EOQ;
		...
		function addSubClass() {
			...
			//sel.name = "Archive[class_id][]";
			//sel.style.width = "160px";
			//for(var class_id in classes) {
			//	var op = document.createElement('option');
			//	op.value = class_id;
			//	op.text = "├───".substring(0, (classes[class_id]['deepth']-1) * 2) + classes[class_id]['class_name'];
			//	sel.appendChild(op);
			//}
			//$('#archive_subclass').append(sel);
			$('select[name="Archive[class_id][]"]:first').clone()
				.appendTo('#archive_subclass')
				.children(':selected')
				.removeAttr('selected');
		}
		
		function delSubClass() {
			$('#archive_subclass select').last().remove();
		}
		...
		$('#game-server-form').submit(function(){
			...
			var o = $("select[name='Archive[class_id][]']");
			var class_is_null = true;
			for(var i in o) {
				if(!isNaN(parseInt(i))) {
					if(o[i].value > 0) {
						class_is_null = false;
			...
			if(class_is_null) {
				alert('请至少选择一个文档栏目！');
				$("select[name='Archive[class_id][]']").eq(0).focus();
				return false;
			}
