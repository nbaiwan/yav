 ...
 * @since 1.0
 */
class Db_CDbCommand extends CComponent
{
	/**
	 ...
	 * Since 1.1.7 it is possible to use a specific mode of data fetching by setting
 	 ...
 	 * for more details.
	 */
	public function __construct(Db_CDbConnection $connection,$query=null)
	{
	...
	public function prepare()
	{
		if($this->_statement==null) {
			try {
				...
				throw new Yaf_Exception('CDbCommand failed to prepare the SQL statement: ' .$e->getMessage(), (int)$e->getCode(), $errorInfo);
			}
		}
	}
	...
	private function queryInternal($method, $mode, $params=array())
	{
		...
		if($this->_connection->query_caching_count>0 && $method!==''
				&& $this->_connection->queryCachingDuration>0
				&& $this->_connection->queryCacheID!==false
				&& ($cache=Yii::app()->getComponent($this->_connection->queryCacheID))!==null) {
			$this->_connection->queryCachingCount--;
			$cacheKey='yii:dbquery'.$this->_connection->connection_string.':'.$this->_connection->username;
		...
		try {
			...
			if($method==='') {
				$result=new Db_CDbDataReader($this);
			} else {
				$mode=(array)$mode;
			...
			*/
			...
			throw new Yaf_Exception("Db_CDbCommand failed to execute the SQL statement: {$message}, Error Message: {$errorInfo}", (int)$e->getCode());
		}
	}
	...
	private function processConditions($conditions)
	{
		...
		throw new Yaf_Exception("Unknown operator \"{$operator}\".");
	}
